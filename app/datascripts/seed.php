...
for ($i = 0; $i < AUTHORS_COUNT; $i++) {
    ...
    $author_name = $i > 0 ? strtolower($faker->name()) : 'Aline DB';
    $author_slug = $slugify->slugify($author_name);
    $author_avatar = $faker->imageUrl(128, 128, 'people', true, $author_name);
    $author_email = $i > 0 ? $faker->unique()->safeEmail : 'alinedebarros@hotmail.fr';
    $author_password = password_hash('1234', PASSWORD_DEFAULT);
    $pdo->exec(<<<SQL
